...
Prefer fonts that are already verified in the bundled catalogs. Use `references/font_catalog.json` for curated recommendations, `references/font_decision_guide.json` for situation-specific role mapping, and `references/fonts_archive_commercial.json` for exhaustive fonts-archive exploration. Do not claim commercial safety for fonts outside these catalogs unless the user explicitly asks you to research and verify new sources.

## Quick Start

1. Read the user's theme, product type, and desired mood.
2. If the request names a specific use case, check `references/font_decision_guide.json` first for a body/heading/code role map.
...
4. Use the selected body and heading fonts unless the codebase already has a strong typography system that should be preserved.
5. Keep the role mapping stable across every screen, section, or slide: once `body`, `heading`, and `code` are chosen, do not swap them around per page.
6. Paste the emitted stylesheet tags or inline style block, plus the CSS variables, into the project.
7. If the page contains code blocks, terminal UI, CLI examples, or developer-product sections, add `NanumGothicCoding` for code.
...
- Use only fonts in `references/font_catalog.json` or `references/fonts_archive_commercial.json` when the user asks for commercially safe Korean webfonts and does not ask for extra research.
...
- If no clear vibe is provided, default to `Wanted Sans Variable` or `Pretendard Variable` for body and `SUITE Variable`, `NanumSquare Neo`, or the same family for headings.
...
Use this when the user wants a broad fonts-archive sweep rather than a curated recommendation. The file includes fonts-archive repos with webfont CSS and explicit commercial-use/open-license signals, while excluding obvious personal-only, noncommercial, purchase-only, or platform-conditional licenses.

### `references/font_decision_guide.json`

Use this when the user asks when to choose which font. It contains situation-first presets such as AI SaaS landing, dense dashboard, soft fintech, food brand, Baemin local campaign, retro poster, kids education, cozy lifestyle, editorial story, premium institution, and public notice. Each scenario includes body, heading, code, candidate font IDs, best-use notes, avoid notes, and a short sample line.
...
Rebuilds `showcase.html` and `index.html` from the curated catalog, fonts-archive catalog, and decision guide. Run this after changing any of those JSON files.
