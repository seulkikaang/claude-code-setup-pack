...
DEFAULT_DECISION_GUIDE = BASE_DIR / "references" / "font_decision_guide.json"
...
TAG_SYNONYMS = {
    ...
    "compact": ["compact", "컴팩트", "촘촘", "좁은", "좁고", "밀도", "정보량", "많이 넣기", "많이 넣는", "많이 담기"],
    ...
    "food": ["food", "푸드", "음식", "식당", "맛집", "배달", "먹거리"],
    "freeform": ["freeform", "자유분방", "자유로운", "엉뚱", "두서없는"],
    ...
    "retro": ["retro", "레트로", "복고", "옛날", "간판", "을지로"],
    "rough": ["rough", "거친", "낡은", "비바람", "오래된"],
    "safe": ["safe", "안전", "무난", "보편적"],
...
PROFILE_DEFS = {
    "product": {
        "label": "제품형/모던 UI",
        "tags": ["modern", "clean", "ui", "product", "startup", "saas", "ai", "dashboard", "tech", "developer", "b2b", "compact"]
    },
    "editorial": {
        "label": "에디토리얼/브랜드 스토리",
    ...
    "playful": {
        "label": "친근/발랄/생활형",
        "tags": ["playful", "cute", "friendly", "community", "family", "wellness", "lifestyle", "cozy", "soft", "handwritten", "youth", "food", "baemin"]
    },
    "impact": {
        "label": "강한 캠페인/프로모션",
...
PROFILE_RECIPES = {
    "product": {
        "body": ["wanted-sans-variable", "suit-variable", "pretendard-variable", "spoqa-han-sans-neo", "goorm-sans", "ibm-plex-sans-kr", "line-seed-sans-kr", "noto-sans-kr", "nanum-gothic", "nanum-square", "nanum-square-neo", "gmarket-sans"],
        "heading": ["suite-variable", "wanted-sans-variable", "nanum-square-neo", "gmarket-sans", "suit-variable", "goorm-sans", "pretendard-variable", "spoqa-han-sans-neo", "ibm-plex-sans-kr", "line-seed-sans-kr", "nanum-square", "noto-sans-kr"],
    ...
    "playful": {
        "body": ["line-seed-sans-kr", "bm-hanna-air", "nanum-square-round", "gowun-dodum", "nanum-square", "goorm-sans", "noto-sans-kr"],
        "heading": ["line-seed-sans-kr", "yg-jalnan", "jua", "bm-kkubulim", "bm-euljiro", "bm-euljiro-10years-later", "bm-hanna-11yrs", "bm-hanna-pro", "yeon-sung", "kirang-haerang", "bm-hanna-air", "nanum-square-round", "single-day", "gaegu", "nanum-pen", "nanum-square"],
        "code": []
    },
    "impact": {
        "body": ["wanted-sans-variable", "pretendard-variable", "gmarket-sans", "spoqa-han-sans-neo", "noto-sans-kr", "nanum-square", "ibm-plex-sans-kr"],
        "heading": ["black-han-sans", "do-hyeon", "bm-euljiro", "bm-euljiro-10years-later", "bm-hanna-11yrs", "bm-hanna-pro", "bm-kkubulim", "yg-jalnan", "kirang-haerang", "suite-variable", "gmarket-sans", "nanum-square-neo", "jua", "wanted-sans-variable", "pretendard-variable"],
        "code": ["nanum-gothic-coding"]
    }
}
...
def load_decision_guide() -> dict[str, Any] | None:
    if not DEFAULT_DECISION_GUIDE.exists():
        return None
    with open(DEFAULT_DECISION_GUIDE, "r", encoding="utf-8") as f:
        return json.load(f)
...
def match_scenario(theme: str, guide: dict[str, Any] | None) -> dict[str, Any] | None:
    if not guide:
        return None
    ...
    candidates = []
    for index, scenario in enumerate(guide.get("scenarios", [])):
        score = 0
        label = normalize_text(scenario["label"])
        scenario_id = normalize_text(scenario["id"].replace("-", " "))
        if label and label.replace(" ", "") in compact:
            score += 8
        if scenario_id and scenario_id.replace(" ", "") in compact:
            score += 8

        primary_tokens = keyword_tokens(f"{scenario['label']} {scenario['id']}")
        for token in primary_tokens:
            if token in normalized:
                score += 2

        context_tokens = keyword_tokens(
            f"{scenario.get('tone', '')} {scenario.get('best_when', '')}"
        )
        ...
        if score >= 3:
            candidates.append((score, -index, scenario))
...
def derive_tags(theme: str) -> list[str]:
    ...
    if "cute" in tags:
        tags.add("playful")
    if "wavy" in tags or "freeform" in tags:
        tags.add("playful")
    if "baemin" in tags:
        tags.update({"food", "playful"})
    if "food" in tags:
        tags.add("playful")
...
def score_font(font: dict[str, Any], tags: set[str], role: str) -> int:
    ...
    if role == "heading":
        ...
        if tags.intersection({"playful", "cute", "handwritten", "family", "community"}) and font_tags.intersection({"playful", "handwritten", "youth", "friendly", "cute"}):
            score += 6
        if tags.intersection({"wavy", "freeform"}) and font_tags.intersection({"wavy", "freeform"}):
            score += 12
        if "retro" in tags and font_tags.intersection({"retro", "street", "local"}):
            score += 13
        elif tags.intersection({"street", "local"}) and font_tags.intersection({"street", "local", "rough"}):
            score += 13
...
def build_scenario_recommendation(
    ...
    font_map = {font["id"]: font for font in catalog["fonts"]}
    body = font_map.get(scenario.get("body"))
    heading = font_map.get(scenario.get("heading"))
    code = font_map.get(scenario.get("code")) if scenario.get("code") else None
    ...
    chosen_ids = {font["id"] for font in selected.values() if font}
    alternatives = [
        font_map[font_id]
        for font_id in scenario.get("font_ids", [])
        if font_id in font_map and font_id not in chosen_ids
    ][:limit]
    ...
    return {
        ...
        "selected": {
            "body": {
                "id": body["id"],
                "name": body["name"],
                "reason": f"{scenario['label']}의 본문 역할. {scenario['best_when']}"
            },
            "heading": {
                "id": heading["id"],
                "name": heading["name"],
                "reason": f"{scenario['label']}의 제목 역할. 톤: {scenario['tone']}."
            },
            "code": None if not code else {
                "id": code["id"],
                "name": code["name"],
                "reason": "코드, 터미널, CLI 예시가 섞일 때 한글과 기호를 선명하게 분리한다."
            }
        },
...
def build_recommendation(catalog: dict[str, Any], theme: str, limit: int) -> dict[str, Any]:
    scenario = match_scenario(theme, load_decision_guide())
    if scenario:
        scenario_recommendation = build_scenario_recommendation(catalog, theme, scenario, limit)
        if scenario_recommendation:
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    if result.get("scenario"):
        parts.append("")
        parts.append("Situation fit")
        parts.append(f"- Tone: {result['scenario']['tone']}")
        parts.append(f"- Best when: {result['scenario']['best_when']}")
        parts.append(f"- Avoid: {result['scenario']['avoid']}")
    parts.append("")
